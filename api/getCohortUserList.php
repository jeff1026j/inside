...
$sql=" SELECT Orders.email,Orders.username, MAX(Orders.order_time) as max_order_time, GROUP_CONCAT(Orders.product_name SEPARATOR ', ')
 FROM  Orders 
     JOIN (SELECT email, EXTRACT(YEAR_MONTH from Min(order_time)) AS cohortDate 
       FROM  Orders 
       GROUP  BY email
       HAVING cohortDate = ?
       ) AS cohorts 
